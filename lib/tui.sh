...
tui_export_key() {
    ...
    local f; f="$(mktemp)"
    {
        ...
        echo -n "RESTIC_PASSWORD="; cat "${RESTIC_PASSWORD_FILE:-/etc/pbo/restic-pass}" 2>/dev/null || echo "<no password file>"
    } > "$f"
    chmod 600 "$f"
    _wt_text "DR key (secret)" "$f"
    if _wt_yesno "DR key" "Save a copy to /root/pbo-dr-key.txt (0600)?"; then
        install -m 600 "$f" /root/pbo-dr-key.txt && _wt_msg "DR key" "Saved: /root/pbo-dr-key.txt\nMove it offline and delete it from this host."
    fi
    rm -f "$f"
}
...
tui_maint() {
    ...
    case "$c" in
        ...
        test)
            ...
            _wt_yesno "Test-restore" "Run a full test-restore of $id (fetch→boot→destroy)?" && _wt_run "Test-restore $id" "$PBO_BIN" test-restore "$id" ;;
    esac
}
...
_tui_selftest() {
    ...
    printf 'PBO_BIN:        '; [[ -x "$PBO_BIN" || -f "$PBO_BIN" ]] && echo "$PBO_BIN" || { echo MISSING; ok=0; }
